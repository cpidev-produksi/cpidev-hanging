    ...
    <div class="dn-body" id="{{ $domId }}-body" style="display:none">
      @foreach($rows as $it)
        <div class="dn-row">
          ...
          @if($canEditDone && $it->hangingForm)
            <a href="{{ route('hanging-forms.show', $it->hangingForm) }}"
               class="dn-btn-edit">Edit</a>
          @endif
...
.dn-btn-summary:hover::before { transform: translateX(100%); }
...
.dn-btn-summary-icon { display: flex; align-items: center; opacity: .9; }
.dn-btn-summary-label { line-height: 1; }
.dn-btn-summary-arrow { display: flex; align-items: center; opacity: .75; transition: transform .15s, opacity .15s; }
.dn-btn-summary:hover .dn-btn-summary-arrow { transform: translate(2px, -2px); opacity: 1; }
</style>
