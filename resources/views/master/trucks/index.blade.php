...
<div class="trucks-page">
    ...
    <div class="page-header">
        <div class="page-title-group">
            ...
            <div>
                <h1 class="page-title">Data Truk</h1>
                <p class="page-subtitle">Kelola seluruh truk</p>
            </div>
        </div>

        <a href="{{ route('master.trucks.create') }}" class="btn-primary">
                    ...
                    <tr>
                        <td>
                            <div class="truck-cell">
                                <div class="avatar">{{ strtoupper(substr($t->no_truck, 0, 1)) }}</div>
                                <span class="truck-name">{{ $t->no_truck }}</span>
                            </div>
                        </td>
                        <td>
                            <code class="mono-badge">{{ $t->plate_number }}</code>
                        </td>
                        <td>
                            @if($t->expedition?->name)
                                <span class="expedition-badge">{{ $t->expedition->name }}</span>
                            @else
                                <span class="text-empty">—</span>
                            @endif
                        </td>
                        <td class="action-cell">
                            <a href="{{ route('master.trucks.edit', $t) }}" class="btn-icon-edit" title="Edit">
                            ...
                            <form method="POST" action="{{ route('master.trucks.destroy', $t) }}" class="inline-form" onsubmit="return confirm('Hapus truk {{ $t->no_truck }}?')">
                                @csrf @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="empty-state">
                            ...
                            <a href="{{ route('master.trucks.create') }}" class="btn-primary-sm">+ Tambah Sekarang</a>
                        </td>
                    </tr>
                @endforelse
                </tbody>
...
/* ===== PAGE LAYOUT ===== */
.trucks-page { max-width: 1100px; margin: 0 auto; padding: 28px 20px; }
...
/* ===== CELLS ===== */
.truck-cell { display: flex; align-items: center; gap: 10px; }
...
.truck-name { font-weight: 600; color: var(--c-text); }
