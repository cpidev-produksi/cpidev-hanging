...
<div class="form-page">
  <div class="page-header">
    <div>
      ...
      <p class="page-subtitle">Input data kedatangan untuk proses hanging</p>
    </div>
    ...
    <div class="grid">
      <div class="form-group">
        ...
        <select name="location" class="form-input">
          <option value="SH01" @selected(old('location')=='SH01')>SH01</option>
      ...
      <div class="form-group">
        ...
        <input type="date" name="process_date" value="{{ old('process_date', date('Y-m-d')) }}" class="form-input">
      </div>
      ...
      <div class="form-group">
        ...
        <select name="size" class="form-input">
          @foreach($sizes as $s)
            <option value="{{ $s }}" @selected((string)old('size')===(string)$s)>{{ $s }}</option>
          @endforeach
        </select>
      </div>
      ...
      <div class="form-group">
        ...
        <input name="driver_name" value="{{ old('driver_name') }}" class="form-input" placeholder="Nama sopir">
      </div>

      <div class="form-group">
        <label class="form-label">Nominal Ekoran Farm</label>
        <input name="farm_fee_amount" type="number" step="0.01" value="{{ old('farm_fee_amount', 0) }}" class="form-input">
      </div>
      ...
      <div class="form-group">
        ...
        <select id="expedition_id" name="expedition_id" class="form-input">
          ...
          @foreach($expeditions as $e)
            <option value="{{ $e->id }}" @selected(old('expedition_id')==$e->id)>{{ $e->name }}</option>
          @endforeach
        </select>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (oldExp) renderPlates(oldExp, oldPlate);
});
</script>
