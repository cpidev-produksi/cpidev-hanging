...
class LiveMonitorController extends Controller
{
    ...
    public function data(string $location)
    {
        ...
        // 2 antrian truk selanjutnya (belum running/done), diurutkan berdasarkan truck_no
        $nextQueue = MonitorControl::query()
            ->where('location', $location)
            ->whereDate('process_date', $today)
            ->where('status', 'draft')
            ->with(['expedition', 'plateNumber', 'farm'])
            ->orderBy('truck_no')
            ->limit(2)
            ->get()
            ->map(function ($mc) {
                return [
                    ...
                    'plate_number' => optional($mc->plateNumber)->plate_number,
                    'expedition_name' => optional($mc->expedition)->name,
                    'farm_name' => optional($mc->farm)->name,
                    'size' => (string) ($mc->size ?? ''),
                ];
            })
            ->values();
