...
class MonitorControlController extends Controller
{
    public function index()
    {
        $items = MonitorControl::query()
            ->with(['truck.expedition','farm'])
            ->latest()
            ->paginate(20);

        return view('transaction.monitor_controls.index', compact('items'));
    }

    public function create()
    {
        $trucks = Truck::query()->with('expedition')->orderBy('no_truck')->get();
        $farms  = Farm::query()->orderBy('name')->get();

        $sizes = [1.2, 1.3, 1.4, 1.5];

        return view('transaction.monitor_controls.create', compact('trucks','farms','sizes'));
    }
    ...
    public function start(MonitorControl $monitorControl)
    {
        ...
        return DB::transaction(function () use ($monitorControl) {
            ...
            return redirect()->route('hanging-forms.show', $form)->with('status', 'Proses dimulai. Silakan input Form Hanging.');
        });
    }
